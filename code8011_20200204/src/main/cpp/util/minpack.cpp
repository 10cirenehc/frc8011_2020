...
#include "util/minpack.h"
using namespace minpack;
using namespace std;
//****************************************************************************80

void minpack::chkder ( int m, int n, double x[], double fvec[], double fjac[],
...
void minpack::dogleg ( int n, double r[], int lr, double diag[], double qtb[],
...
//
  ...
  for ( k = 1; k <= n; k++ )
  {
    ...
    if ( temp == 0.0 )
    {
      ...
      for ( i = 1; i <= j; i++ )
      {
        temp = minpack::r8_max ( temp, fabs ( r[l-1] ) );
        l = l + n - i;
      }
      temp = epsmch * temp;
      if ( temp == 0.0 )
...
//
  temp = ( 1.0 - alpha ) * minpack::r8_min ( sgnorm, delta );
  for ( j = 0; j < n; j++ )
  {
...
double minpack::enorm ( int n, double x[] )
...
void minpack::fdjac1 ( void fcn ( int n, double x[], double f[], int &iflag ),
...
//
  ...
  eps = sqrt ( minpack::r8_max ( epsfcn, epsmch ) );
  msum = ml + mu + 1;
//
...
/*
void minpack::fdjac2 ( void fcn ( int m, int n, double x[], double fvec[], int &iflag ), 
...
int minpack::hybrd ( void fcn ( int n, double x[], double fvec[], int &iflag ),
  int n, double x[],
...
//
    iflag = 2;
    minpack::fdjac1 ( fcn, n, x, fvec, fjac, ldfjac, iflag, ml, mu, epsfcn, wa1, wa2 );
...
//
    minpack::qrfac ( n, n, fjac, ldfjac, false, iwa, 1, wa1, wa2 );
//
...
//
    if ( mode == 1 )
    {
      for ( j = 0; j < n; j++ )
      {
        diag[j] = minpack::r8_max ( diag[j], wa2[j] );
      }
    }
//
...
//
      ...
      pnorm = minpack::enorm ( n, wa3 );
//
//  On the first iteration, adjust the initial step bound.
//
      if ( iter == 1 )
      {
        delta = minpack::r8_min ( delta, pnorm );
      }
//
...
//
      ...
      else
      {
        ...
        if ( p5 <= ratio || 1 < ncsuc )
        {
          delta = minpack::r8_max ( delta, pnorm / p5 );
        }
...
//
      ...
      if ( p1 * minpack::r8_max ( p1 * delta, pnorm ) <= epsmch * xnorm )
      {
        info = 3;
        return info;
      }
...
//
      minpack::r1updt ( n, n, r, lr, wa1, wa2, wa3 );
      minpack::r1mpyq ( n, n, fjac, ldfjac, wa2, wa3 );
...
//
  ...
  info = minpack::hybrd ( fcn, n, x, fvec, xtol, maxfev, ml, mu, epsfcn, wa, mode,
...
int minpack::i4_max ( int i1, int i2 )
...
int minpack::i4_min ( int i1, int i2 )
...
void minpack::qform ( int m, int n, double q[], int ldq )
...
void minpack::qrfac ( int m, int n, double a[], int lda, bool pivot, int ipvt[],
...
//
  ...
  for ( j = 0; j < n; j++ )
  {
    acnorm[j] = minpack::enorm ( m, a+j*lda );
    rdiag[j] = acnorm[j];
    wa[j] = rdiag[j];
    if ( pivot )
    {
...
//
    ajnorm = minpack::enorm ( m - j, a+j+j*lda );
...
//
      for ( k = j + 1; k < n; k++ )
      {
        ...
        if ( pivot && rdiag[k] != 0.0 )
        {
          ...
          rdiag[k] = rdiag[k] * sqrt ( minpack::r8_max ( 0.0, 1.0 - temp * temp ) );
          if ( p05 * ( rdiag[k] / wa[k] ) * ( rdiag[k] / wa[k] ) <= epsmch )
          {
            rdiag[k] = minpack::enorm ( m - 1 - j, a+(j+1)+k*lda );
            wa[k] = rdiag[k];
          }
        }
      }
...
void minpack::qrsolv ( int n, double r[], int ldr, int ipvt[], double diag[], 
...
void minpack::r1mpyq ( int m, int n, double a[], int lda, double v[], double w[] )
...
bool minpack::r1updt ( int m, int n, double s[], int ls, double u[], double v[],
  double w[] )
...
double minpack::r8_max ( double x, double y )
...
double minpack::r8_min ( double x, double y )
...
double minpack::r8_uniform_01 ( int &seed )
...
double* minpack::r8mat_mm_new ( int n1, int n2, int n3, double a[], double b[] )
...
void minpack::r8mat_print ( int m, int n, double a[], std::string title )
...
//
{
  minpack::r8mat_print_some ( m, n, a, 1, 1, m, n, title );
...
//
  ...
  {
    j2hi = j2lo + INCX - 1;
    j2hi = minpack::i4_min ( j2hi, n );
    j2hi = minpack::i4_min ( j2hi, jhi );
...
//
    i2lo = minpack::i4_max ( ilo, 1 );
    i2hi = minpack::i4_min ( ihi, m );
...
void minpack::r8vec_print ( int n, double a[], std::string title )
